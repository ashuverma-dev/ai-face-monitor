...
function Settings({ preferences, onSave, showToast, adminUsername }: { preferences: AppPreferences; onSave: (preferences: AppPreferences) => void; showToast: (message: string) => void; adminUsername: string }) {
  ...
  return <>
    ...
    <div className="page-body settings-grid">
      ...
      <section className="panel setting-card alerts-card"><span className="eyebrow">Daily notifications</span><h2>Brevo Email Alerts</h2><p>Brevo se daily summary mein absent students aur low attendance ki information milegi.</p><label className="switch-row"><span>Enable email alerts</span><input type="checkbox" checked={alerts.enabled} onChange={(event) => setAlerts((value) => ({ ...value, enabled: event.target.checked }))} /></label><label>Low attendance below<input type="number" min="1" max="100" value={alerts.low_percentage} onChange={(event) => setAlerts((value) => ({ ...value, low_percentage: Number(event.target.value) }))} />%</label><input type="email" value={alerts.email_recipient} onChange={(event) => setAlerts((value) => ({ ...value, email_recipient: event.target.value }))} placeholder="Alert receive karne ka email" /><label className="switch-row"><span>Include absent student list</span><input type="checkbox" checked={alerts.send_absent} onChange={(event) => setAlerts((value) => ({ ...value, send_absent: event.target.checked }))} /></label><small>Brevo service: {alerts.email_available ? "connected" : "fresh API key needed"}</small><div className="settings-actions"><button className="primary-button" onClick={() => void saveAlerts()}>Save Email Alerts</button><button className="ghost-button" onClick={() => void testAlerts()}>Send Test Email</button></div></section>
